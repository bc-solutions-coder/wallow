 ...
 */
export function readSessionRef(event: H3Event, config: BffConfig): string | null {
...
function clearSession(event: H3Event, config: BffConfig): void {
  ...
  deleteCookie(event, csrfCookieName(config.cookieName), csrfCookieOpts(config));
  ...
  return {
    login: defineEventHandler(async (event: H3Event): Promise<void> => {
      ...
      const returnTo: string =
        typeof query.returnTo === "string" && query.returnTo !== "" ? query.returnTo : "/";
