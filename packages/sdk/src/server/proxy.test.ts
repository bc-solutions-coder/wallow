 ...
 */
function makeFakeStore(stored: BffSession | null, options: FakeStoreOptions = {}): FakeStore {
  ...
  const store: SessionStore = {
    ...
    async withRefreshLock<T>(_ref: string, fn: () => Promise<T>): Promise<T | undefined> {
...
describe("ensureFreshSession", () => {
  it("returns the session unchanged without acquiring the refresh lock when still fresh", async () => {
    ...
    const result: BffSession = await ensureFreshSession(session, config, store, "ref-fresh");
  ...
  it("performs the refresh inside store.withRefreshLock, bumps version, and writes the new session", async () => {
    ...
    vi.mocked(refreshTokens).mockImplementation(async (): Promise<TokenResponse> => {
      expect(insideLock).toBe(true);
      return tokens;
    });

    const result: BffSession = await ensureFreshSession(session, config, store, "ref-1");
  ...
  it("re-reads the session from the store instead of refreshing when the lock is held by a concurrent request", async () => {
    const config: BffConfig = makeConfig("https://refresh-contended.example.com");
    const session: BffSession = makeSession({
    ...
    const result: BffSession = await ensureFreshSession(session, config, store, "ref-2");
  ...
  it("throws when the lock is held but the store no longer has the session", async () => {
    ...
    await expect(ensureFreshSession(session, config, store, "ref-3")).rejects.toThrow();
  });
  ...
  it("throws when the access token is expired and there is no refresh token", async () => {
    ...
    await expect(ensureFreshSession(session, config, store, "ref-4")).rejects.toThrow();
...
describe("createApiProxy", () => {
  ...
  it("silently refreshes an expired session, re-seals the cookie, and forwards the new token", async () => {
    ...
    const upstreamCall = fetchMock.mock.calls.find(
      (call): boolean =>
        !String(call[0]).includes(".well-known") && !String(call[0]).includes("/connect/token"),
    );
    expect(upstreamCall).toBeDefined();
    const init = upstreamCall?.[1] as RequestInit;
    const headers: Headers = new Headers(init.headers);
...
describe("createApiProxy forwarded headers", () => {
  ...
  async function forwardedHeadersFor(
    ...
    const res: Response = await makeHandle(config)(request);
    expect(res.status).toBe(200);
  ...
  it("appends the peer address to the upstream X-Forwarded-For", async () => {
    const headers: Headers = await forwardedHeadersFor(
      "https://proxy-xff-new.example.com",
      {},
      undefined,
      "1.2.3.4",
    );
  ...
  it("preserves an inbound chain and appends this hop's peer to its end", async () => {
    const headers: Headers = await forwardedHeadersFor(
      "https://proxy-xff-chain.example.com",
      { "x-forwarded-for": "198.51.100.9, 70.41.3.18" },
      undefined,
      "1.2.3.4",
    );

    // Ends with this hop's peer — the entry the API's rate limiter pops. With no
    // proxy trusted the inbound chain is carried but never promoted.
  ...
  it("never believes a client-IP header the caller sent in place of the socket", async () => {
    const headers: Headers = await forwardedHeadersFor("https://proxy-xff-strip.example.com", {
      "x-wallow-client-ip": "1.2.3.4",
    });

    expect(headers.has("x-wallow-client-ip")).toBe(false);
  ...
  it("still forwards the content-type and accept allowlist alongside them", async () => {
    const headers: Headers = await forwardedHeadersFor(
      "https://proxy-xff-allowlist.example.com",
  ...
  it("does not leak the inbound Cookie header upstream", async () => {
    const headers: Headers = await forwardedHeadersFor(
      "https://proxy-xff-cookie.example.com",
      {},
      undefined,
      "1.2.3.4",
    );
 ...
 */
function stubFetchScript(attempts: readonly FetchAttempt[]): ReturnType<typeof vi.fn> {
  ...
  const fetchMock: ReturnType<typeof vi.fn> = vi.fn(
    (input: unknown, init: RequestInit): Promise<Response> => {
      ...
      if (handler === undefined) {
        return Promise.reject(new Error(`unexpected upstream attempt #${attempt}`));
      }
...
/** RFC 7807 body as the .NET API emits it (machine code as a top-level `code`). */
function problem(
  status: number,
...
function bearerOf(fetchMock: ReturnType<typeof vi.fn>, attempt: number): string | null {
...
function makeForwardRequest(overrides: Partial<ForwardRequest> = {}): ForwardRequest {
  return {
...
describe("forceRefreshSession", () => {
  it("refreshes under the lock even when the access token is not near expiry", async () => {
    ...
    const result: BffSession = await forceRefreshSession(session, config, store, "ref-force");
  ...
  it("throws when there is no refresh token to force a refresh with", async () => {
    const config: BffConfig = makeConfig("https://force-no-refresh.example.com");
...
describe("forwardWithResilience", () => {
  ...
  it("surfaces a ApiFailure when the replayed request is rejected with a second 401", async () => {
    const config: BffConfig = makeConfig("https://forward-401-twice.example.com");
    const session: BffSession = makeSession({
      accessToken: "rejected-access",
      refreshToken: "the-refresh-token",
    });
    ...
    const error: ApiFailure = await forwardWithResilience(
      makeForwardRequest(),
      config,
      store,
      session,
      ...
      (thrown: unknown): ApiFailure => thrown as ApiFailure,
    );
  ...
  it("does not retry a 401 when the session has no refresh token", async () => {
    const config: BffConfig = makeConfig("https://forward-401-norefresh.example.com");
    const session: BffSession = makeSession({ refreshToken: undefined });
    const { store }: FakeStore = makeFakeStore(session);
    const fetchMock: ReturnType<typeof vi.fn> = stubFetchScript([
      respond((): Response => problem(401, { title: "Unauthorized" })),
    ...
    await expect(
      forwardWithResilience(makeForwardRequest(), config, store, session, "ref-401-norefresh"),
  ...
  it("treats a 3xx redirect to the login page as an auth failure and replays once", async () => {
    const config: BffConfig = makeConfig("https://forward-login-redirect.example.com");
    const session: BffSession = makeSession({
      accessToken: "rejected-access",
      refreshToken: "the-refresh-token",
      version: 5,
  ...
  it("surfaces a ApiFailure when the replayed request is redirected to the login page again", async () => {
    const config: BffConfig = makeConfig("https://forward-redirect-twice.example.com");
    const session: BffSession = makeSession({
      refreshToken: "the-refresh-token",
    });
    const { store }: FakeStore = makeFakeStore(session);
  ...
  it("does not treat a non-login 3xx as an auth failure", async () => {
    const config: BffConfig = makeConfig("https://forward-redirect-other.example.com");
    const session: BffSession = makeSession({
      refreshToken: "the-refresh-token",
    });
    const { store, calls }: FakeStore = makeFakeStore(session);
  ...
  it("waits for Retry-After and replays the request once on a 429", async () => {
    ...
    const { store, calls }: FakeStore = makeFakeStore(session);
    const fetchMock: ReturnType<typeof vi.fn> = stubFetchScript([
      respond((): Response => problem(429, { title: "Too Many Requests" }, { "retry-after": "1" })),
      respond((): Response => new Response(JSON.stringify({ ok: true }), { status: 200 })),
    ]);
  ...
  it("bounds the Retry-After wait to MAX_RETRY_AFTER_MS", async () => {
    vi.useFakeTimers();
    const config: BffConfig = makeConfig("https://forward-429-bounded.example.com");
    const session: BffSession = makeSession();
    const { store }: FakeStore = makeFakeStore(session);
    const fetchMock: ReturnType<typeof vi.fn> = stubFetchScript([
      respond(
        (): Response => problem(429, { title: "Too Many Requests" }, { "retry-after": "3600" }),
      ),
      respond((): Response => new Response(JSON.stringify({ ok: true }), { status: 200 })),
    ]);
  ...
  it("surfaces a ApiFailure when the replayed request is throttled again", async () => {
    vi.useFakeTimers();
    const config: BffConfig = makeConfig("https://forward-429-twice.example.com");
    const session: BffSession = makeSession();
    const { store }: FakeStore = makeFakeStore(session);
    const fetchMock: ReturnType<typeof vi.fn> = stubFetchScript([
      respond((): Response => problem(429, { title: "Too Many Requests" }, { "retry-after": "1" })),
    ...
    const settled: Promise<ApiFailure> = pending.then(
      (): never => {
        throw new Error("expected forwardWithResilience to reject");
      },
      (thrown: unknown): ApiFailure => thrown as ApiFailure,
  ...
  it("raises a 504 Transport.Timeout ApiFailure when the forward exceeds FORWARD_TIMEOUT_MS", async () => {
    ...
    const settled: Promise<ApiFailure> = forwardWithResilience(
      makeForwardRequest(),
      config,
      store,
      session,
  ...
  it("raises a Client.UnrecognizedResponse ApiFailure when a non-OK response is not problem details", async () => {
    ...
    expect(error.code).toBe(ClientErrorCode.CLIENT_UNRECOGNIZED_RESPONSE);
  });

  it("replays the request body on a retried attempt", async () => {
    const config: BffConfig = makeConfig("https://forward-replay-body.example.com");
    const session: BffSession = makeSession({
      refreshToken: "the-refresh-token",
    });
    const { store }: FakeStore = makeFakeStore(session);
    ...
    const fetchMock: ReturnType<typeof vi.fn> = stubFetchScript([
      respond((): Response => problem(401, { title: "Unauthorized" })),
      respond((): Response => new Response(JSON.stringify({ ok: true }), { status: 201 })),
    ]);
...
describe("createApiProxy resilience", () => {
  it("recovers from a reactive 401 and re-seals the rotated session into the cookie", async () => {
    const config: BffConfig = makeConfig("https://proxy-reactive-401.example.com");
    const session: BffSession = makeSession({
      accessToken: "rejected-access",
      refreshToken: "the-refresh-token",
      expiresAt: Date.now() + 3_600_000,
...
describe("createApiProxy CSRF", () => {
  ...
  it("rejects an empty CSRF header even when the session has no token", async () => {
    ...
    const { store }: FakeStore = makeFakeStore(makeSession({ csrfToken: undefined }));
    const fetchMock: ReturnType<typeof vi.fn> = stubUpstreamOk();
    const handle = makeHandle(config, store);
  ...
  it.each(UNSAFE_METHODS)(
    ...
    async (method: string) => {
      ...
      expect(res.status).toBe(200);
      expect(fetchMock).toHaveBeenCalledTimes(1);
      expect(String(fetchMock.mock.calls[0][0])).toContain("https://api.example.com/tenants");
    },
  );

  it.each(PROXIED_SAFE_METHODS)("does not gate a %s on a CSRF token", async (method: string) => {
    const config: BffConfig = makeConfig(`https://csrf-safe-${method.toLowerCase()}.example.com`);
    const { store }: FakeStore = makeFakeStore(makeSession());
    const fetchMock: ReturnType<typeof vi.fn> = stubUpstreamOk();
    const handle = makeHandle(config, store);
