...
`unwrap`/`createAuthClient`/`createMfaClient` layer, the three per-app facade singletons, and
the imperative `login()`/`getUser()` browser helpers (superseded by `loginRedirect()` links
and `getCurrentUser`/`currentUserQuery`; only the CSRF-gated `logout()` stays imperative)
