...
`unwrap`/`createAuthClient`/`createMfaClient` layer, the three per-app facade singletons,
the imperative `login()`/`getUser()` browser helpers (superseded by `loginRedirect()` links
and `getCurrentUser`/`currentUserQuery`; only the CSRF-gated `logout()` stays imperative), and
the browser claim-bag readers (`claims.ts` — `getRoles`/`hasRole`/`isAdmin`/`isOperator`/
`isGlobalAdmin`/`getOrgId`/`getOrgName`; the app boundary reads the typed `CurrentUser` through
`@bc-solutions-coder/auth`'s `hasRole`/`hasPermission`/`isAdmin`, and id_token decoding is the
server entry's own `server/claims.ts`) are **deleted outright**, not deprecated. Three things enforce that, and a change touching
