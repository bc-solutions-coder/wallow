 ...
 *      `beforeLoad` primer, and the role/permission helpers;
 *   2. re-exports of the SDK's route guards and claim helpers, so an app's auth
 *      imports come from ONE package instead of being split across two. They are
 *      re-exported by reference, not wrapped: `isAdmin` from here IS the SDK's
 *      `isAdmin`.
 *
