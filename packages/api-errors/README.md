...
| `parseRetryAfter(header)`                     | `Retry-After` delta-seconds or HTTP-date as whole seconds from now (`undefined` when absent or unparseable), for a caller passing a parsed body to `toApiFailure` with `context.retryAfter`.                                                |
| `resolveFailureMessage(error, options?)`      | Call-site `messages` → app `registry` → first nonblank `unmatched` message (4xx `Validation.Failed`) → shipped copy per code → `detail` (4xx, API codes only) → shipped copy per status → `fallback` → a generic sentence. Always a string. |
