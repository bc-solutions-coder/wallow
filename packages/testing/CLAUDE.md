...
- **`location` is `[Unforgeable]` in real Chromium.** `vi.stubGlobal("location", …)` cannot shadow it,
  ...
  runner blames, so an opt-in guard cannot catch the file that forgot. All five browser projects are
  wired — `packages/navigation`, `packages/forms`, `packages/ui`, `apps/wallow-web` and
  `apps/wallow-auth`; `packages/ui`'s `storybook` project takes it through
  `.storybook/preview.tsx`'s `beforeEach`/`afterEach` exports instead, because `storybookTest()`
  builds that project itself and never reads `browserSetupFiles`. **A spec asserting a DELIBERATE
