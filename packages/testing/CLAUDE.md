...
| `./router-stub` (`src/router-stub.ts`)                                     | Inside a **spec** that mocks the router | `assertRouterStubApplied` — a `beforeEach` guard for specs that `vi.mock("@tanstack/react-router")`: the factory marks its stub `Link` inline (`Object.assign(fn, { wallowRouterStub: true })`, since a hoisted factory cannot reference imports), and the guard fails the file by NAME if the mocker ever serves the real module. The stub's anchor also carries `data-router-stub="true"` for `./navigation-escape`'s forensics. |
| `./console-guard` (`src/console-guard.ts`)                                 | A browser project's **setup file**      | `installConsoleGuard` + `assertNoConsoleNoise` — wraps (never replaces) `console.error`/`console.warn` so a React key collision, an update outside `act` or a boundary catch fails the test that produced it instead of scrolling past a green run. A spec that drives an error path on purpose reads its entries back with `consumeConsoleNoise` / `expectConsoleError`; anything unread still fails.                             |
| `./network-escape` (`src/network-escape.ts`)                               | A browser project's **setup file**      | `installNetworkEscapeGuard` + `assertNoNetworkEscape` — anything reaching `globalThis.fetch` is traffic no `createSdkHarness()` owns, so it is answered with a 503 naming the request rather than forwarded. Unguarded it hangs in CI and passes against a live database locally. `consumeNetworkEscapes` reads back a request a spec provoked deliberately; Vite/Vitest's own same-origin paths pass through.                     |
...
- **`location` is `[Unforgeable]` in real Chromium.** `vi.stubGlobal("location", …)` cannot shadow it,
  ...
  "a spec that forgot to assert still fails" true: an escape nobody consumes is still sitting in
  the record when the project `afterEach` runs, and it fails the test that leaked it. This is what
  `apps/wallow-auth` rides on — its screens hand the browser off for real, six of them assigning
  `globalThis.location.href`/`.assign`/`.replace` at seven sites to finish an OIDC flow, and nine
  specs assert those hand-offs through these helpers.
