    ...
    ILogger<PluginLoader> logger)
{
    public PluginRegistryEntry LoadPlugin(PluginManifest manifest, string pluginsBasePath)
    {
        ...
        List<Type> pluginTypes = assembly.GetTypes()
            .Where(t => typeof(IWallowPlugin).IsAssignableFrom(t) && !t.IsAbstract)
            .ToList();

        if (pluginTypes.Count == 0)
        {
            loadContext.Unload();
            throw new PluginLoadException(manifest.Id, $"No IWallowPlugin implementation found in '{manifest.EntryAssembly}'.");
        }

        if (pluginTypes.Count > 1)
        {
            loadContext.Unload();
            throw new PluginLoadException(manifest.Id, $"Multiple IWallowPlugin implementations found in '{manifest.EntryAssembly}'. Expected exactly one.");
        }

        IWallowPlugin plugin = (IWallowPlugin)(Activator.CreateInstance(pluginTypes[0])
