...
- **May** reference `Foundry.Shared.Kernel` and `Foundry.Shared.Contracts`.
...
- **Depends on**: All module `.Api` projects (Identity, Notifications, Billing; Email commented out), `Foundry.Shared.Kernel`, `Foundry.Shared.Contracts`.
- **Depended on by**: Test projects (`Foundry.Api.Tests`, `Foundry.Tests.Common`) reference this via `WebApplicationFactory<Program>`.
