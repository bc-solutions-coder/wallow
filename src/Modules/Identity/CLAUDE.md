...
- **Application** (`Foundry.Identity.Application`): Defines `IKeycloakAdminService` and `IKeycloakOrganizationService` interfaces, plus DTOs. Keycloak proxy operations (user/org CRUD) are called directly from controllers -- no CQRS handlers. Service accounts, SSO, and SCIM DO use proper command/query handlers.
