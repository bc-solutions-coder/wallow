...
namespace Foundry.Messaging.Infrastructure.Services;
    ...
    ITenantContext tenantContext) : IMessagingQueryService
{
    public async Task<bool> IsParticipantAsync(Guid conversationId, Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = """
            SELECT EXISTS(
                ...
                INNER JOIN messaging.conversations c ON c.id = p.conversation_id
    ...
    public async Task<int> GetUnreadConversationCountAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        const string sql = """
            ...
            INNER JOIN messaging.participants p
                ON p.conversation_id = m.conversation_id
                AND p.user_id = @UserId
            INNER JOIN messaging.conversations c
        ...
        string sql = cursorMessageId.HasValue
            ? """
              SELECT m.id AS "Id",
                     ...
                     m.status AS "Status"
              FROM messaging.messages m
              INNER JOIN messaging.conversations c ON c.id = m.conversation_id
              INNER JOIN messaging.participants p
                  ON p.conversation_id = m.conversation_id
                  AND p.user_id = @UserId
              WHERE m.conversation_id = @ConversationId
                AND c.tenant_id = @TenantId
                AND m.sent_at < (SELECT sent_at FROM messaging.messages WHERE id = @CursorMessageId)
