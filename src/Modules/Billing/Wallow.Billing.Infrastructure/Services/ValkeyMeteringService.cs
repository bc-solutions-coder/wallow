...
/// </summary>
public sealed partial class ValkeyMeteringService(
    ...
    ILogger<ValkeyMeteringService> logger) : IMeteringService
{
    ...
    public Task IncrementAsync(string meterCode, decimal value = 1, Dictionary<string, string>? dimensions = null)
    {
        ...
        _ = incrementTask.ContinueWith(
            t => LogMeteringIncrementFailed(logger, t.Exception!, key),
        ...
        _ = expireTask.ContinueWith(
            t => LogMeteringExpireFailed(logger, t.Exception!, key),
            CancellationToken.None,
            TaskContinuationOptions.OnlyOnFaulted,
            TaskScheduler.Default);
        ...
        _ = indexTask.ContinueWith(
            t => LogMeteringIndexAddFailed(logger, t.Exception!, key),
            CancellationToken.None,
            TaskContinuationOptions.OnlyOnFaulted,
            TaskScheduler.Default);
    ...
    public async Task<QuotaCheckResult> CheckQuotaAsync(string meterCode)
    {
        ...
        string? planCode = await subscriptionQueryService.GetActivePlanCodeAsync(tenantId.Value, CancellationToken.None);

        // Get effective quota (tenant override > plan default)
        QuotaDefinition? quota = await quotaRepository.GetEffectiveQuotaAsync(
            meterCode,
            planCode,
            CancellationToken.None);
        ...
        int currentPercent)
    {
        ...
        foreach (int threshold in _thresholds)
        {
            if (currentPercent >= threshold && lastTriggered < threshold)
            {
                ...
                MeterDefinition? meter = await meterRepository.GetByCodeAsync(meterCode, CancellationToken.None);
                string displayName = meter?.DisplayName ?? meterCode;

                // Raise domain event
                await messageBus.PublishAsync(new QuotaThresholdReachedEvent(
                    tenantId,
                    meterCode,
                    displayName,
                    currentUsage,
