...
| `src/app/routes/bff/$.ts`, `src/app/routes/api/$.ts`, `src/app/routes/health.ts` | File-based server routes (single `ANY`/`GET` handler each) delegating to `src/app/lib/bff.ts`. See BFF wiring below.                                                                                                                                                                                                                                   |
| `src/app/lib/`                                                                   | Server-only plumbing: `bff.ts`, the lazily-built `createWallowBffServer()` host exposing `handleBffRequest`/`handleApiRequest`/`handleHealthRequest`. It lives in `app/` and not `shared/` precisely because nothing else may import it.                                                                                                               |
...
| `src/shared/`                                                                    | `components/` (`DashboardLayout`, `DashboardNav`, `PublicLayout`, `ready-indicator`), `lib/` (`error-text`, `request-origin`, `site-links`, `use-is-desktop`), `stores/` (`ui-store`), `testing/` (spec harnesses). The `QueryClient` factory lives in `@bc-solutions-coder/query` and the current-user query in `@bc-solutions-coder/auth`, not here. |
...
export `server.handlers` alongside (or instead of) a UI component. `src/app/lib/bff.ts`
...
`ANY` (or `GET`) handler apiece, reaching `src/app/lib/bff.ts` through a dynamic
