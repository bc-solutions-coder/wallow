...
| `src/lib/`                                                           | Shared plumbing: `wallow-sdk.ts` (client configurator, registered with the SDK query bootstrap), `bff.ts` (lazily-built `createWallowBffServer()` host exposing `handleBffRequest`/`handleApiRequest`/`handleHealthRequest`). The `QueryClient` factory itself lives in `@bc-solutions-coder/query` (`createQueryClient`), not here, and the current-user query the route gates read lives in `@bc-solutions-coder/auth`. |
