...
| `src/shared/`                                                                    | `components/` (`DashboardLayout` and its `dashboard-destinations` manifest, `SignOut`, `PublicLayout`, `ready-indicator`), `lib/` (`log`, `site-links`, `fork-links`). What used to sit beside them is now shared: the dashboard shell is `@bc-solutions-coder/navigation`, the `QueryClient` factory `@bc-solutions-coder/query`, the current-user query `@bc-solutions-coder/auth`, failure parsing and message resolution `@bc-solutions-coder/api-errors`, and `PageContainer` + `SimpleSelect` `@bc-solutions-coder/ui`. |
