...
| `src/shared/`                                                                    | `components/` (`DashboardLayout` and its `dashboard-destinations` manifest, `SignOut`, `PublicLayout`, `ready-indicator`), `lib/` (`error-text`, `page-container`, `site-links`), `testing/` (spec harnesses). The dashboard shell itself is `@bc-solutions-coder/navigation`, the `QueryClient` factory `@bc-solutions-coder/query`, and the current-user query `@bc-solutions-coder/auth` — none of them here. |
