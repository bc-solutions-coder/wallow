 ...
 * lifecycle, path dispatch, and handing the inbound request through intact so
 * the SDK can read the peer address off it.
 *
...
describe("the wallow-web BFF host", () => {
  ...
  it("forwards the inbound request itself rather than a copy", async () => {
    ...
    // survive undici's copy constructor, and a copy would lose the `ip` the SDK
    // resolves the client address from.
    const forwarded = mocks.handleApi.mock.calls[0]?.[0] as (Request & { ip?: string }) | undefined;
