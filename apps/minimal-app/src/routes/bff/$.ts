...
/**
 * `/bff/**` — the OIDC tunnel: `login`, `callback`, `user`, `logout`, and the
 * front- and back-channel logout receivers.
 *
 ...
 * method-filtered route would swallow that as a 404 and hide the contract.
 * Sub-paths the tunnel does not own answer 404 from the handler itself.
 *
 * `src/lib/bff.server.ts` is reached through a dynamic import, not a top-level
 * one: it pulls in `@bc-solutions-coder/sdk/server` (node:crypto, openid-client),
 * and every route module is a member of the route tree the CLIENT graph imports.
