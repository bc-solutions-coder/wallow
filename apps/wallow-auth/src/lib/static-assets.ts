/**
 * Built-client asset reads for the wallow-auth standalone host (Wallow-vec7.5.1.1).
 *
 ...
 *
 * Kept out of `server.ts` because that file is a listening side effect from its
 * first line and cannot be imported under test; this is the part with rules
 * worth pinning. Shape follows `apps/wallow-web/server.ts`'s static handler
 * (content-type map, root containment, miss fallback) with one deliberate
 * difference: a miss here returns `undefined` rather than a 404, because the
 * caller must hand the path to the router, which owns the real 404-vs-200-vs-
 * redirect decision for every app route.
 */
