...
beforeAll(async (): Promise<void> => {
  rootDir = await mkdtemp(join(tmpdir(), "wallow-auth-static-"));
  ...
  await writeFile(join(rootDir, "..", "wallow-auth-static-secret.txt"), "secret");
...
describe("createStaticAssetReader", () => {
  ...
  it("yields nothing for the root path so the router renders it", async () => {
    // `/` is the `/` -> `/login` redirect the router owns, not a directory read.
  ...
  it("refuses to read outside the asset root", async () => {
    await expect(read("/../wallow-auth-static-secret.txt")).resolves.toBeUndefined();
  });
});
