...
def legacy_source(client, registry, current, repository, image_id, record, now=None):
    ...
    if set(record) != fields or type(record['schema']) is not int or record['schema'] != 1 or not matches(r'[a-f0-9]{40}', record['source_sha']):
    ...
    if current['digest'] != images.get(image_id):
        return None
    if not all(positive_integer(record[key]) for key in ('run_id', 'run_attempt', 'workflow_id')) or record['tag'] != record['source_sha'][:7]:
    ...
    expected = {'id': record['run_id'], 'run_attempt': record['run_attempt'], 'workflow_id': record['workflow_id'],
                'path': '.github/workflows/deploy.yml', 'head_sha': record['source_sha'], 'head_branch': 'main',
