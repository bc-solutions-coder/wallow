...
class ReleaseGitHub(GitHub):
    ...
    def upload(self, release_id, name, body):
        if not positive_integer(release_id) or not (name in ('wallow-release-origin-v1.json', 'wallow-release-selection-v1.json', 'wallow-package-publication-v1.json', 'wallow-image-publication-v1.json') or matches(r'wallow-alias-(?:image|package)-[1-9][0-9]*-[1-9][0-9]*\.json', name) or matches(r'wallow-recovery-v1-[1-9][0-9]*-[1-9][0-9]*\.json', name) or matches(r'wallow-(?:release|package|image|recovery)-endorsement-v1-[1-9][0-9]*-[1-9][0-9]*-[1-9][0-9]*\.json', name)) or not isinstance(body, bytes) or not 0 < len(body) <= RECORD_LIMIT:
