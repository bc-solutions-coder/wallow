...
def retain(client, release_id, name, payload, current):
    ...
    if existing:
        ...
        if not endorsed(client, release_id, existing):
            endorsement = f"{endorsement_prefix(name)}{existing['asset_id']}-{current[0]['run_id']}-{current[0]['run_attempt']}.json"
...
def endorsed(client, release_id, receipt):
    if receipt['originating_job_success']:
        return True
    prefix = f"{endorsement_prefix(receipt['name'])}{receipt['asset_id']}-"
