...
| `packages/lint/`   | `@bc-solutions-coder/lint` — Wallow's own oxlint JS-plugin rules (`wallow/*`), registered at the repo root (`no-source-tests` repo-wide) and by five nested configs (both apps, `ui`, `forms`, `navigation`) |
