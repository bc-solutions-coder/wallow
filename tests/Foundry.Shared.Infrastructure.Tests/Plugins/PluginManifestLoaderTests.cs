...
{
    private readonly string _tempDir = Path.Combine(Path.GetTempPath(), $"foundry-test-{Guid.NewGuid()}");
    ...
    private static void WriteManifest(string pluginDir, object manifest)
    {
        ...
        File.WriteAllText(Path.Combine(pluginDir, "foundry-plugin.json"), json);
    }
}
