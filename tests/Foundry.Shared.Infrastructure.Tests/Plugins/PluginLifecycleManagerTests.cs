...
public class PluginLifecycleManagerTests
{
    ...
    public PluginLifecycleManagerTests()
    {
        ...
        {
            PluginsDirectory = Path.Combine(Path.GetTempPath(), "foundry-test-plugins")
        });
    ...
    public async Task EnablePluginAsync_PluginInInstalledState_TransitionsToEnabled()
    {
        ...
        IFoundryPlugin plugin = Substitute.For<IFoundryPlugin>();
    ...
    {
        string pluginsDir = Path.Combine(Path.GetTempPath(), $"foundry-discover-{Guid.NewGuid()}");
    ...
    {
        string pluginsDir = Path.Combine(Path.GetTempPath(), $"foundry-discover-empty-{Guid.NewGuid()}");
    ...
    {
        string nonExistentPath = Path.Combine(Path.GetTempPath(), $"foundry-no-exist-{Guid.NewGuid()}");
    ...
    {
        string pluginsDir = Path.Combine(Path.GetTempPath(), $"foundry-discover-multi-{Guid.NewGuid()}");
        ...
        try
        {
            ...
            string manifestB = """
                ...
                """;
            await File.WriteAllTextAsync(Path.Combine(pluginADir, "foundry-plugin.json"), manifestA);
            await File.WriteAllTextAsync(Path.Combine(pluginBDir, "foundry-plugin.json"), manifestB);
...
#pragma warning restore CA2000
    ...
    {
        string pluginsDir = Path.Combine(Path.GetTempPath(), $"foundry-log-discover-{Guid.NewGuid()}");
    ...
    {
        string pluginsDir = Path.Combine(Path.GetTempPath(), $"foundry-log-empty-{Guid.NewGuid()}");
        Directory.CreateDirectory(pluginsDir);
    ...
    {
        string pluginsDir = Path.Combine(Path.GetTempPath(), $"foundry-log-multi-{Guid.NewGuid()}");
