...
public class PluginLoaderTests
{
    ...
    public void LoadPlugin_AssemblyWithNoPluginImplementation_ThrowsPluginLoadException()
    {
        ...
        try
        {
            // Create a minimal valid assembly with no IFoundryPlugin implementation
            ...
            act.Should().Throw<PluginLoadException>()
                .Where(e => e.PluginId == "no-impl-plugin"
                    && e.Message.Contains("No IFoundryPlugin implementation found"));
        }
        finally
        {
            Directory.Delete(tempDir, recursive: true);
    ...
    {
        // Build a dynamic assembly with two concrete IFoundryPlugin implementations.
        // Both are minimal stubs — PluginLoader only needs GetTypes() to find them.
        Type pluginInterface = typeof(IFoundryPlugin);
        ...
        for (int i = 1; i <= 2; i++)
        {
            ...
            ctorIl.Emit(OpCodes.Ldstr, "1.0.0");                 // MinFoundryVersion
    ...
    public void LoadPlugin_ValidPlugin_RegistersAndReturnsEntry()
    {
        ...
        try
        {
            ...
            result.Instance.Should().BeAssignableTo<IFoundryPlugin>();
        ...
        try
        {
            ...
            act.Should().Throw<PluginLoadException>()
                .Where(e => e.PluginId == "multi-plugin"
                    && e.Message.Contains("Multiple IFoundryPlugin implementations found"));
        }
        finally
        {
            Directory.Delete(tempDir, recursive: true);
