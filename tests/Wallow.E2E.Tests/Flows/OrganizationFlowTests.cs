...
public sealed class OrganizationFlowTests : AuthenticatedE2ETestBase
{
    ...
    /// Clears cookies first to ensure a fresh session, then performs the full OIDC login.
    /// </summary>
    private async Task LoginAsUserAsync(string email, string password)
    {
        // Clear all cookies to end the current session before logging in as a different user
