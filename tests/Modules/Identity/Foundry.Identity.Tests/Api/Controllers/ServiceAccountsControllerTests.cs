...
public class ServiceAccountsControllerTests
{
    ...
    public async Task Create_WithValidRequest_ReturnsCreated()
    {
        ...
        Foundry.Identity.Api.Contracts.Requests.CreateServiceAccountRequest request = new("Backend", "Backend service", _billingReadScope);
        ServiceAccountCreatedResult createdResult = new(newId, "client-backend", "secret-123", "https://kc/token", _billingReadScope);
        _serviceAccountService.CreateAsync(Arg.Any<Foundry.Identity.Application.DTOs.CreateServiceAccountRequest>(), Arg.Any<CancellationToken>())
    ...
    {
        ServiceAccountMetadataId newId = ServiceAccountMetadataId.New();
        Foundry.Identity.Api.Contracts.Requests.CreateServiceAccountRequest request = new("Svc", "Description", _singleScope);
        ServiceAccountCreatedResult createdResult = new(newId, "c1", "s1", "t1", _singleScope);
        Foundry.Identity.Application.DTOs.CreateServiceAccountRequest? capturedRequest = null;
        _serviceAccountService.CreateAsync(Arg.Do<Foundry.Identity.Application.DTOs.CreateServiceAccountRequest>(r => capturedRequest = r), Arg.Any<CancellationToken>())
            .Returns(createdResult);
