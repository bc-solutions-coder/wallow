...
using Wallow.Storage.Application.Telemetry;

namespace Wallow.Storage.Tests.Application.Telemetry;
...
public class StorageModuleTelemetryTests
{
    ...
    {
        StorageModuleTelemetry.OperationsTotal.Name.Should().Be("wallow.storage.operations_total");
    }

    [Fact]
    public void BytesTransferred_HasCorrectName()
    {
        StorageModuleTelemetry.BytesTransferred.Name.Should().Be("wallow.storage.bytes_transferred");
    }

    [Fact]
    public void OperationDuration_HasCorrectName()
    {
        StorageModuleTelemetry.OperationDuration.Name.Should().Be("wallow.storage.operation_duration");
    }
}
