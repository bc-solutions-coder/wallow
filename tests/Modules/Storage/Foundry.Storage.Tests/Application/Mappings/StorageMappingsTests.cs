...
namespace Foundry.Storage.Tests.Application.Mappings;
...
public class StorageMappingsTests
{
    ...
    [Fact]
    public void ToDto_StorageBucket_WithEmptyContentTypesJson_ReturnsEmptyList()
    {
        StorageBucket bucket = StorageBucket.Create(
            TenantId.New(), "bucket", allowedContentTypes: []);

        BucketDto dto = bucket.ToDto();

        dto.AllowedContentTypes.Should().NotBeNull();
        dto.AllowedContentTypes.Should().BeEmpty();
    }

    [Fact]
    public void ToDto_StorageBucket_WithSingleContentType_DeserializesCorrectly()
    {
        StorageBucket bucket = StorageBucket.Create(
            TenantId.New(), "bucket", allowedContentTypes: ["application/pdf"]);

        BucketDto dto = bucket.ToDto();

        dto.AllowedContentTypes.Should().HaveCount(1);
        dto.AllowedContentTypes.Should().Contain("application/pdf");
    }

    [Fact]
    public void ToDto_StorageBucket_WithDeleteRetentionAction_MapsCorrectly()
    {
        RetentionPolicy retention = new(30, RetentionAction.Delete);
        StorageBucket bucket = StorageBucket.Create(
            TenantId.New(), "ephemeral", retention: retention);

        BucketDto dto = bucket.ToDto();

        dto.Retention.Should().NotBeNull();
        dto.Retention!.Days.Should().Be(30);
        dto.Retention.Action.Should().Be("Delete");
    }

    [Fact]
    public void ToDto_StoredFile_WithEmptyMetadataString_MapsAsIs()
    {
        TenantId tenantId = TenantId.New();
        StorageBucket bucket = StorageBucket.Create(tenantId, "bucket");
        StoredFile file = StoredFile.Create(
            tenantId, bucket.Id, "file.txt", "text/plain", 100,
            "key", Guid.NewGuid(), metadata: "");
    ...
    [Fact]
    public void ToDto_StorageBucket_WithWildcardContentTypes_DeserializesCorrectly()
    {
        StorageBucket bucket = StorageBucket.Create(
            TenantId.New(), "bucket", allowedContentTypes: ["image/*", "application/pdf"]);

        BucketDto dto = bucket.ToDto();

        dto.AllowedContentTypes.Should().HaveCount(2);
        dto.AllowedContentTypes.Should().Contain("image/*");
        dto.AllowedContentTypes.Should().Contain("application/pdf");
    }
}
