...
namespace Foundry.Billing.Tests.Api.Extensions;
...
public class ResultExtensionsTests
{
    ...
    {
        Result result = Result.Failure(Error.NotFound("Invoice", "123"));
    ...
    {
        Result result = Result.Failure(Error.NotFound("Invoice", "test"));
    ...
    public void ToCreatedResult_WhenSuccess_ReturnsCreatedResult()
    {
        ...
        IActionResult actionResult = result.ToCreatedResult("/api/billing/invoices/1");

        CreatedResult created = actionResult.Should().BeOfType<CreatedResult>().Subject;
        created.StatusCode.Should().Be(StatusCodes.Status201Created);
        created.Value.Should().Be("created-value");
        created.Location.Should().Be("/api/billing/invoices/1");
    }
    ...
    public void ToCreatedResult_WhenSuccess_SetsLocationHeader()
    {
        ...
        IActionResult actionResult = result.ToCreatedResult("/api/billing/invoices/abc-123");

        CreatedResult created = actionResult.Should().BeOfType<CreatedResult>().Subject;
        created.Location.Should().Be("/api/billing/invoices/abc-123");
    }
