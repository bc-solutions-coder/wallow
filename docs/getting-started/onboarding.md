...
**How it works:**
1. JWT contains `org` claim (organization ID)
...
**Short answer:** ASP.NET Core Identity is the source of truth for user accounts.

The Identity module manages users, organizations, and roles through ASP.NET Core Identity with OpenIddict for token issuance. CQRS would add ceremony without benefit for these operations.
...
Core architecture:
- **Architecture Assessment:** `docs/architecture/assessment.md` — Design decisions and patterns
- **Developer Guide:** `docs/getting-started/developer-guide.md` — How to work in the codebase
- **Deployment Guide:** `docs/operations/deployment.md` — Server setup, CI/CD, Docker
