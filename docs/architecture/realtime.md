...
| Event broadcasts (inquiry updates) | SSE | One-way with permission filtering |
...
```
GET /events?subscribe=inquiries,notifications
...
|--------|-------------|---------|
| `SendToTenantAsync` | All tenant members should see it | Inquiry status changed |
