...
```
src/
  Foundry.Api/                          # Host — wires modules, middleware pipeline, Program.cs
  Modules/
    Identity/
  ...
  Shared/
    ...
    Foundry.Shared.Infrastructure/      # Cross-cutting infrastructure (Auditing, BackgroundJobs, Workflows)
```
...
Tenants map to Keycloak Organizations within a single `foundry` realm. Data isolation uses EF Core global query filters on `TenantId`.
...
Foundry is a base platform. To build a product:

1. **Fork** the repository.
2. **Rename** — Update solution name, namespaces, Docker image names, Keycloak realm/clients as needed.
3. **Add modules** — Create new modules under `src/Modules/` following the four-layer structure. Register them in `FoundryModules.cs`.
...
1. Create `src/Modules/{Name}/Foundry.{Name}.Domain` (class library)
2. Create `src/Modules/{Name}/Foundry.{Name}.Application` (class library, references Domain)
3. Create `src/Modules/{Name}/Foundry.{Name}.Infrastructure` (class library, references Application)
4. Create `src/Modules/{Name}/Foundry.{Name}.Api` (class library, references Application)
...
7. Register module in `src/Foundry.Api/FoundryModules.cs`
8. Create test project under `tests/Modules/{Name}/`

**Note:** Wolverine automatically discovers handlers in all `Foundry.*` assemblies and uses conventional RabbitMQ routing — no manual exchange/queue configuration needed.
