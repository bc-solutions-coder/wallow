...
| **Integration Tests** | Test API endpoints and cross-cutting concerns | Real PostgreSQL, Redis via Testcontainers |
...
│   ├── Fixtures/                   # Test fixtures (Database, Redis)
...
Tests use `TestAuthHandler` instead of real OAuth2 authentication:
...
## 8. Event Sourcing and Saga Tests (Removed)
...
- **Never use real OAuth2 flows in tests** - Always use `TestAuthHandler`
