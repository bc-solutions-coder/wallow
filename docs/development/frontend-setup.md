...
When forking Wallow, customize the user-facing identity through configuration rather than code changes.

### Branding Customization

Edit `branding.json` in the repository root to set your product name, icon, tagline, and color theme. This single file drives all branding across the Auth and Web boundaries:
...
Place your custom icon in the `wwwroot` directory of `Wallow.Auth` (and `Wallow.Web` if applicable).
...
All layouts inject `IOptions<BrandingOptions>` and read the same configuration. Auth pages (login, register, password reset, email verification) all use `AuthLayout`.
...
### CORS for Fork Frontends

After forking, update the CORS allowed origins to match your frontend URL:
