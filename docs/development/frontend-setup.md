...
server — an app's backend-facing surface is just **server routes** under `src/routes/`,
each delegating to a preset from `@bc-solutions-coder/sdk/server`.
...
(UI routes and server routes side by side), components under `src/components/`, and a single
`vite.config.ts` that serves dev and emits the production bundle:
...
│   ├── start.ts, router.tsx, routeTree.gen.ts
├── vite.config.ts
...
- **Reach a Node-only module through a dynamic `import()`** when the preset pulls
  ...
  `apps/wallow-web/src/lib/bff.ts` is the reference.
...
- **Server routes and hosting** — which prefixes it serves and which SDK preset sits
  behind them (`src/routes/**` + `src/lib/`), plus its own `vite.config.ts`,
