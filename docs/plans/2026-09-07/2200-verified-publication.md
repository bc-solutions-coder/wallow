...
Hosted full Pages acceptance exposed a real archive-compatibility defect: preparation omitted directory headers. Private attempts `34182300013` and `34182527638` correctly retained failed intents/progress after service rejection. A tiny custom-name probe passed (`34182622788`), while the full directory-free archive with a fresh source identity failed (`34182888542`). Adding normalized root/parent directory headers and `./` file paths, preserving every file byte, passed actual public Pages deployment/readback (`34182958901`).

Preparation now emits those directory headers before files. The actual DocFX artifact reproduces the accepted TAR exactly: 47,974,400 bytes, SHA-256 `d8c6be40fabd8e63e61dd74a9993a655598d919dcb09cb20774a481064597f89`, all 1,170 file bytes preserved. Nine site behavior tests pass, including the regression that failed before this correction. The prior failed intent remains immutable; the next full private acceptance uses a new docs producer instead of weakening same-source content conflict checks.

### Immutable package versions and durable readback receipts

The independent `ENABLE_PACKAGE_PUBLISH` path now connects read-only release preparation, the `package-publish` main-only environment, and a separate contents-write receipt finalizer. It uses existing durable release origins and pinned producer selections even when Release Please is disabled. Preparation rescans the selected producer's sealed dependency inputs against the current policy and retains the original packed bytes. Destination checks require the configured GitHub owner scope and repository URL; unconfigured forks remain valid for other capabilities.

Before the first npm write, every catalog-owned runtime, optional and peer dependency must resolve to an authorized release and match exact registry tarball bytes, or appear earlier in the same authorized batch. Range evaluation uses the fixed npm installation's bundled [official node-semver implementation](https://github.com/npm/node-semver/blob/main/README.md) from an empty directory, without project installation or lifecycle execution. External dependencies remain bound to the validated lockfile and fresh Trivy scan. Publishing checks SHA256 and SHA512 after an actual registry download. Its content-derived `validated-` dist-tag is staging only; latest, major and minor alias progression remains a separate step.

The separate finalizer stores immutable package/readback/producer identities in the corresponding Release. A successful durable receipt allows a completed internal dependency to be verified from registry bytes after its original Actions artifact expires. Failed finalizer recovery first revalidates the original successful writer's sealed progress, preserves the original receipt bytes and producer, and adds a successful endorsement. If that incomplete receipt's writer evidence has expired, ordinary retry fails and requires explicit recovery.

Behavior checks exercise real sealed TAR and packed-package handoffs, all dependency reads before writes, missing dependency and altered-seal rejection, exact policy/receipt/scan binding, and failed-finalizer adoption. The handoff test exposed a plain-TAR/gzip mismatch; preparation now uses an explicit strict plain-TAR reader while producer package archives retain their gzip contract. Hosted package transport, genuine release authority and this complete workflow path still require acceptance before enabling the real repository flag.

## Hosted Pages and package transport acceptance

The complete Pages writer passed in disposable private repository run `34183480631`, using registered producer `34183219515/1` at `7487675357ed8dfb34c0e096b6d1d70e220e0768`. It authenticated the earlier failed intents, deployed all 1,170 files, and verified the public index. The retained progress seal and an independent live index download passed. The normalized TAR SHA256 is `d8c6be40fabd8e63e61dd74a9993a655598d919dcb09cb20774a481064597f89`; the 6,796-byte index SHA256 is `c20c61f62407df8c9298b18e533c19f54ab69ee15d8b1b16b0cc28064f99d3ee`.

Identical retry `34183890842` verified the same producer and site, recording intent `6320106370` after initial successful intent `6320037166`. Older-producer retry `34183905694` passed its Pages job with sealed `skip-older` progress and no new intent. API deployment history still ends at `6320106370`. This proves the actual protected writer's deployment, identical retry, and older-source rejection. Real repository Pages cutover and removal of its temporary legacy bootstrap remain pending.

The reviewed package implementation is commit `12a5180e990d1c9ef5f5a72446e330f96cf4fcb1`. Disposable npm run `34183988695` exercised that exact transport and passed initial upload, downloaded SHA256/SHA512 verification, identical retry, conflicting-content rejection, and no lifecycle execution. Version `0.0.0-acceptance.34183988695.1` has SHA256 `3772459364af0c404c7bd86284cbe2c93250acb38a76f3d5c09f72cfca9e0af5`. Integrated package release authorization and durable receipt acceptance remain pending; transport success does not establish genuine Release Please provenance. Evidence is linked from issue #283, comment `5578729268`.

### Authenticated immutable release images

The image capability now includes bounded per-release discovery, preparation, immutable publication and a separate receipt finalizer. Discovery verifies durable Release Please origins and selected producer receipts. Each matrix item uses an exact positive release ID in its job and artifact names and repeats API authorization. Preparation scans all three image bundles and every supported platform from the selected producer; a newer triggering main run never substitutes its images. Completed authenticated receipts are skipped during automatic reconciliation without fetching expired historical producer artifacts. Explicit retries still require the pinned source artifacts and fresh scans; missing artifacts require the separate recovery path.

The writer reuses the existing verified Skopeo copy and complete manifest/config/layer readback operation. It writes full-SHA and exact version references to the same deterministic OCI index bytes, preserving child images. No nightly, latest, major or minor alias is changed by this release writer. The per-release contents-write finalizer records exact image digests and successful protected writer evidence on the corresponding Release. Failed-finalizer adoption revalidates original successful writer progress and preserves the original payload before adding an endorsement. Build-metadata release versions fail explicitly rather than being normalized into a registry tag.

Behavior fixtures exercise real sealed three-bundle/two-platform preparation-to-writer handoffs, immutable conflicts and identical retries, exact per-release plan/seal/job selection, changed policy/producer rejection, complete bounded discovery, expired-input avoidance for completed receipts, and partial receipt adoption. Hosted dynamic matrix job names, genuine release authorization and full workflow acceptance remain pending. Existing main-image, package and Pages paths retain their separate flags and credential boundaries.

### Stable release alias preparation and promotion

The image and package capability paths now prepare and promote stable release aliases after durable immutable publication receipts. Images use `latest`, `X`, and `X.Y`; packages use `latest`, `major-X`, and `minor-X.Y`, because npm rejects numeric range-like dist-tags. Prereleases remain immutable-only, and build metadata fails explicitly. Manual retries for another valid component skip the unrelated capability before selecting its environment.

Read-only preparation enumerates authenticated origins, pinned producer selections and successful immutable receipts, then verifies actual registry bytes. It compares both component versions and main ancestry. Unknown, conflicting or incomparable alias history fails closed; older attempts skip newer aliases. Every component image is checked before any mutation. Changing image targets receive fresh scans from exact registry blobs; changing package targets require current-policy scans of the pinned dependency inputs and exact internal dependency readiness. Expired package target inputs require explicit recovery. Completed dependency identities and unchanged aliases do not force historical build-artifact downloads.

The existing isolated main-only image-publish and package-publish environments perform fixed registry operations with job tokens. GHCR read-only preparation explicitly requests pull scope; it cannot mutate manifests. Writers reauthorize the exact successful preparation and observed alias state immediately before mutation, and require exact readback. npm dist-tag promotion accepts the CLI's real plaintext response only as process output; registry tag and tarball readback establish success. The global workflow queue serializes these writers without providing compare-and-swap against external package writers.

Separate contents-write jobs retain append-only per-invocation observations on the corresponding Releases. These records bind the exact successful writer, sealed progress artifact, immutable publication receipt and verified or skipped outcomes. They do not authorize releases or replace immutable receipts. Partial failures retain diagnostic progress, and a new invocation rechecks actual registry state before continuing. Real sealed archive, API, registry HTTP and orchestration fixtures cover unknown state, changed observations, prereleases, older attempts, missing fresh scans, partial mutation, exact retry and progress retention. Hosted stable-alias promotion and legacy release-alias migration remain acceptance work; no real repository flag is enabled by this slice.
