...
Main CI run 34193191714 exposed a telemetry deletion concurrency failure. PR #294 addresses it with row locking and state refresh. Its deterministic regression failed before the fix; all 284 Identity integration tests passed locally after the fix. Hosted CI for commit `96f963a0` is run 34196298415 and remains pending at this checkpoint. Release PR #291 must refresh after that fix merges and pass its new required checks before release acceptance continues.
